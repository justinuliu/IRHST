...
from river.evaluate.mass_estimator import MassEstimator


class HSTBranch(tree.base.Branch):
    def __init__(self, left, right, feature, threshold, l_mass, r_mass, mass_estimator=None):
        super().__init__(left, right)
        self.feature = feature
        self.threshold = threshold
        self.l_mass = l_mass
        self.r_mass = r_mass
        self.mass_estimator = mass_estimator
        self.counter = 0
...
class HSTLeaf(tree.base.Leaf):
    def __init__(self, mass_estimator: MassEstimator = None, **kwargs):
        super().__init__()
        self.r_mass = 0
        self.l_mass = 0
        self.mass_estimator = mass_estimator
...
def make_padded_tree(limits, height, padding, rng=random, mass_estimator=None, **node_params):
    if height == 0:
        return HSTLeaf(mass_estimator=mass_estimator.clone() if mass_estimator else None, **node_params)
    ...
    left = make_padded_tree(
        limits=limits, height=height - 1, padding=padding, rng=rng, mass_estimator=mass_estimator, **node_params
    )
    limits[on] = tmp
    ...
    return HSTBranch(left=left, right=right, feature=on, threshold=at,
                     mass_estimator=mass_estimator.clone() if mass_estimator else None, **node_params)
...
class HalfSpaceTrees(anomaly.base.AnomalyDetector):
    ...
    def __init__(
            ...
            seed: int = None,
            mass_estimator: MassEstimator = None,
            leaf_scoring=False
    ):
        ...
        self.mass_estimator = mass_estimator
        self.leaf_scoring = leaf_scoring
    ...
    def size_limit(self):
        ...
        if self.window_size > 0:
            ...
            return 0.1 * self.counter  # incremental HS-Tree

    @property
    def _max_score(self):
        """The largest potential anomaly score."""
        if self.mass_estimator:
            n = 1
        ...
        if self.leaf_scoring:
            return self.n_trees * n
        else:
            return self.n_trees * n * (2 ** (self.height + 1) - 1)
    ...
    def learn_one(self, x):
        ...
        if not self.trees:
            self.trees = [
                make_padded_tree(
                    ...
                    r_mass=0,
                    l_mass=0,
                    mass_estimator=self.mass_estimator
                )
        ...
        for t in self.trees:
            for node in t.walk(x):
                if node.mass_estimator:
                    for _ in range(self.counter - node.counter):
                        node.mass_estimator.add(0)
                    node.mass_estimator.add(1)
                    node.counter = self.counter + 1
                if self.window_size > 0:
                    node.l_mass += 1
                else:
                    node.r_mass += 1  # incremental HS-Tree
        ...
        if self.leaf_scoring:
            for t in self.trees:
                ...
                if self.mass_estimator:
                    ...
                    score += node.mass_estimator.estimation()
                else:
                    score += node.r_mass
        else:
            for t in self.trees:
                for depth, node in enumerate(t.walk(x)):
                    if self.mass_estimator:
                        for _ in range(self.counter - node.counter):
                            node.mass_estimator.add(0)
                        node.counter = self.counter
                        score += node.mass_estimator.estimation() * 2 ** depth
                    else:
